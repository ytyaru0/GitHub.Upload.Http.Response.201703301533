...
# `GetLinkGetLink(r, rel='next')`を`r.links['next']['url']`に置換したい
#find . -type f | grep .py | xargs grep "Link"
find . -type f | grep .py | xargs grep "self.response.GetLinkNext(r)"
find . -type f | grep .py | xargs sed -i "s/self.response.GetLinkNext(r)/self.response.Headers.Link.Next(r)/g"
find . -type f | grep .py | xargs grep "self.response.Headers.Link.Next(r)"
